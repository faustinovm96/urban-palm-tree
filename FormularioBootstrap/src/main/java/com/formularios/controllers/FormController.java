...
public class FormController {
	...
	public String form(Model model) {
		...
		empleado.setValorOculto("Hola Mundo");
		empleado.setPais(new Pais(3, "ARG", "Argentina"));
		empleado.setRoles(Arrays.asList(new Role(3, "ROLE_MODERATOR", "Moderador"), new Role(1, "ROLE_ADMIN", "Administrador")));
	...
	@GetMapping("/ver")
	public String ver(@SessionAttribute(name = "empleado", required = false) Empleado empleado, 
			Model model, SessionStatus status) {
		
		if(empleado == null) {
			return "redirect:/form";
		}
		model.addAttribute("titulo", "Resultado POST");
